...
\begin{definition}
    Углом между векторами $\Vec{a}$ и $\Vec{b}$ называется: $\phi=\arccos \left( \dfrac{(\Vec{a};\Vec{b})}{|\Vec{a}\parallel\Vec{b}|}\right)\Longleftrightarrow \cos \phi = \dfrac{(\Vec{a};\Vec{b})}{|\Vec{a}\parallel\Vec{b}|}$, где $|\Vec{a}|=\sqrt{(a,a)}.$
\end{definition}
