...
\begin{proof}
	...
	\begin{center}
		\begin{tikzpicture}[scale=.8]
            ...
            \tkzDrawPoints(A, B, C, M, Ma, Mc, Nc, Na)
		\end{tikzpicture}
	\end{center}
...
\begin{proof}
	...
	\begin{center}
		\begin{tikzpicture}
            ...
            \tkzDrawPoints(A, B, C, D, E, F, M, K, L, P, Q)
		\end{tikzpicture}
	\end{center}
	...
	$$\triangle PKM\sim \triangle BLM\Longrightarrow \frac{PK}{KM}=\frac{BL}{ML};\quad\triangle QKM\sim \triangle DLM\Longrightarrow \frac{KQ}{KM}=\frac{DL}{ML}$$
...
\begin{proof}
	...
	$$\triangle ABD\sim \triangle EBC\Longrightarrow AD\cdot BC=BD\cdot EC;\enspace\triangle ABE\sim \triangle DBC\Longrightarrow AB\cdot CD=BD\cdot AE$$
...
\begin{proof}
	...
	$$\triangle AC_1B_1\sim \triangle BC_1D\Longrightarrow \frac{BC_1}{C_1A}=\frac{BD}{AB_1};\quad\triangle CA_1B_1\sim \triangle BA_1D\Longrightarrow \frac{BA_1}{A_1C}=\frac{BD}{CB_1}$$
...
\begin{proof}
	...
	$$\frac{S_{AOB}}{S_{AOC}}=\frac{BA_1}{A_1C};\quad\frac{S_{AOB}}{S_{BOC}}=\frac{AB_1}{B_1C};\quad\frac{S_{AOC}}{S_{BOC}}=\frac{AC_1}{C_1B}$$
...
\begin{proof}
	...
	\begin{center}
		\begin{tikzpicture}
            ...
            \tkzDrawPoints(A, B, C, Ka, Kb, Kc, N, Tb, Tc)
		\end{tikzpicture}
	\end{center}
...
\begin{proof}
	...
	\begin{center}
		\begin{tikzpicture}
            ...
            \tkzDrawPoints(O1, O2, K, L, M, N, A, B, C, D, Q1, Q2)
		\end{tikzpicture}
	\end{center}
	Пусть даны окружности $\omega_1(O_1;R_1)$ и $\omega_2(O_2;R_2)$, касательные $O_1K$, $O_1L$, $O_2M$, $O_2N$, точки $A$, $B$, $C$, $D$ пересечения касательных с окружностями и точки $Q_1$ и $Q_2$ -- середины $AB$ и $CD$ соответственно. $\triangle AO_1B$ равнобедренный, откуда $\angle AQ_1O_1$ прямой, то есть:
...
\end{theorem}
    ...
    \begin{center}
        ...
        \draw (-5,3) -- (5,3);
        \coordinate[label=above:$C$] (C) at (-4.5,3);
        \coordinate[label=above:$A$] (A) at (-1.5,3);
        \coordinate[label=above:$B$] (B) at (4.5,3);
...
\end{theorem}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.75]
            \tkzDefPoint(60:4.5){B}
            \tkzDefPoint(-145:4.5){A}
            \tkzDefPoint(-35:4.5){C}
            \tkzDefPoint(0,0){O}
            \tkzDefPoint(0,-5.1621){K}
            ...
            \draw[dashed, name path = line2] ($(-145:4.5)!(60:4.5)!(-35:4.5)$) -- (60:4.5);
            \draw[name intersections={of=line1 and line2}] (intersection-1) node[right] {$H$};
            \coordinate (Hc) at ($(-145:4.5)!(60:4.5)!(-35:4.5)$);
            \coordinate (int) at (intersection-1);
            \tkzDrawPoints(A, B, C, K, O, int)
            \draw[-{Stealth[scale = 1.5]}] (0,0) -- (int);
...
\end{theorem}
    ...
    \begin{center}
        ...
        \tkzMarkSegment[mark=|](P,F2)
        \tkzDrawPoints(F1, F2, X, Y, P, F1', F2')
        \end{tikzpicture}
    \end{center}
...
\subsection{Свойства параболы}
    ...
    \begin{proof}
        ...
        \begin{center}
            ...
            \tkzDefCircle[circum](F,X',Y')
            \tkzGetPoint{P} \tkzGetLength{rayon}
            \draw[dotted, line width = 0.2mm] (P) circle(\rayon);
            \draw (P) node[right] {$P$};
            \begin{scope}
                \clip (-1,-.6) rectangle (1,0.7);
                \draw[dashed] (P) -- ++(0,-10);
    ...
    \hfill
        \begin{center}
            ...
            \def\r{3}
                ...
                \draw (P') node[below left] {$P\,'$};
                \draw[dashed] ($(Pc)!1.68!(Pb)$) -- ($(Pb)!1.5!(Pc)$);
                \draw (P) -- (A);
                \draw[dotted, line width=.25mm] (P) -- (Pc);
                \draw (P) -- (B');
                \draw (B) -- (B');
                \draw (B) -- (H');
                \draw (P) -- (H');
                ...
                \tkzDrawPoints(A, B, C, P, P, Pb, Pc, B', H', P', H)
            \end{tikzpicture}
        \end{center}
    ...
    \begin{proof}
        ...
        \begin{center}
            ...
            \begin{scope}
                ...
                \draw[domain=-4:4, variable=\x, samples=100, dotted, line width=.25mm] plot ({(\x)},{(\x*0.6)^2+0.5});
            ...
            \tkzDrawPoints(F, A, B, C, Fa, Fb, H, O)
        \end{tikzpicture}
        \end{center}
...
\begin{proof}
    ...
    \begin{center}
        ...
        \draw[name path = line1] (K) circle(\rayon);
            ...
            \tkzDrawPoints(A, B, C, B1, A0, C0, Mb)
    \end{tikzpicture}
    \end{center}
...
\begin{proof}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9]
            ...
            \tkzDrawPoints(A, B, C, A0, La, Lc, C0, I, C0')
    \end{tikzpicture}
    \end{center}
...
\begin{definition}
    ...
    \begin{align*}
        1.\,\,&\forall X\in \R^2:\,X\neq0\enspace\exists!\,Y:\,X,\,Y \text{ -- симметричны относительно } \omega\\
...
\subsection{Проективная геометрия}

\begin{definition}
    Проективной плоскостью называется множество, элементы которого -- точки и прямые, причём для них выполнены аксиомы:
    \begin{enumerate}
        \item Для любых двух точек существует прямая, проходящая через них.
        \item Любые две различные прямые пересекаются по одной точке.
        \item Существуют четыре точки, никакие три из которых не лежат на одной прямой.
        \item (Принцип двойственности) При замене в утверждениях проективной геометрии точек на прямые и наоборот, они останутся верными.
    \end{enumerate}
\end{definition}
\begin{definition}
    Двойным отношением точек $A,\,B,\,C,\,D$, лежащих на одной прямой, называется число:
    $$(A,B;C,D)=\frac{\vv{AC}}{\vv{CB}}:\frac{\vv{AD}}{\vv{DB}}$$
\end{definition}
\begin{definition}
    Гармонической четвёркой называется четыре точки, двойной отношение которых равно $-1$.
\end{definition}
\begin{definition}
    Двойным отношением прямых $a,\,b,\,c,\,d$, пересекающихся в одной точке, называется число:
    $$(a,b;c,d)=\frac{\sin\angle(\vv{a};\vv{c})}{\sin\angle(\vv{c};\vv{b})}:\frac{\sin\angle(\vv{a};\vv{d})}{\sin\angle(\vv{d};\vv{b})}$$
\end{definition}
\begin{statement}
    Пусть прямые $a,\,b,\,c,\,d$, пересекающиеся в точке $O$, пересекают прямую $l$ в точках $A,\,B,\,C,\,D$ соответственно, тогда:
    $$(a,b;c,d)=(A,B;C,D)$$
\end{statement}
\begin{proof}
    \hfill
    \begin{center}
        \begin{tikzpicture}
            \tkzDefPoints{0/0/A,1.5/0/B,4/0/C,6/0/D,3.5/4/O}
            \draw ($(A)!-.2!(O)$) -- ($(A)!1.2!(O)$);
            \draw ($(B)!-.2!(O)$) -- ($(B)!1.2!(O)$);
            \draw ($(C)!-.2!(O)$) -- ($(C)!1.2!(O)$);
            \draw ($(D)!-.2!(O)$) -- ($(D)!1.2!(O)$);
            \draw ($(A)!-.2!(D)$) -- ($(A)!1.2!(D)$);
            \tkzLabelSegment[above left](A,O){$a$}
            \tkzLabelSegment[below right](B,O){$b$}
            \tkzLabelSegment[below left](C,O){$c$}
            \tkzLabelSegment[above right](D,O){$d$}
    ...
    \end{center}
    \begin{multline*}
        |(A,B;C,D)|=\left|\frac{\vv{AC}}{\vv{CB}}:\frac{\vv{AD}}{\vv{DB}}\right|=\frac{S_{AOC}}{S_{COB}}:\frac{S_{AOD}}{S_{DOB}}=\\=\frac{OC\cdot OA\cdot \sin\angle(\vv{a};\vv{c})}{OC\cdot OB\cdot \sin\angle(\vv{b};\vv{c})}:\frac{OA\cdot OD\cdot \sin\angle(\vv{a};\vv{d})}{OD\cdot OB\cdot \sin\angle(\vv{b};\vv{d})}=(a,b;c,d)
    \end{multline*}
\end{proof}
\begin{lemma}[Свойство двойных отношений]
    Двойные отношения сохраняются при центральной проекции:
    \begin{multicols}{2}
        \begin{itemize}[itemsep=.1pt]
            \item Прямой на прямую.
            \item Прямой на окружность из точки на этой окружности.
            \item Окружности на прямую из точки на этой окружности.
            \item Окружности на окружность из точки их пересечения.
        \end{itemize}
    \end{multicols}
\end{lemma}
\begin{theorem}[Паппа]
    Пусть на двух прямых взяты точки $A$, $B$, $C$ и $A_1$, $B_1$, $C_1$ соответственно. Пусть прямые $AB_1$, $CA_1$ и $BC_1$ пересекаются с прямыми $A_1B$, $C_1A$ и $B_1C$ в точках $D$, $E$ и $F$ соответственно. Тогда $D$, $E$ и $F$ лежат на одной прямой.
\end{theorem}
\begin{proof}
    \hfill
    \begin{center}
        \begin{tikzpicture}
            \tkzDefPoints{0/0/X,7/0/C1,5/4/C}
            \tkzDefPointWith[linear,K=.3](X,C1) \tkzGetPoint{A1}
            \tkzDefPointWith[linear,K=.7](X,C1) \tkzGetPoint{B1}
            \tkzDefPointWith[linear,K=.45](X,C) \tkzGetPoint{A}
            \tkzDefPointWith[linear,K=.75](X,C) \tkzGetPoint{B}
            \tkzInterLL(A,B1)(A1,B) \tkzGetPoint{D}
            \tkzInterLL(C,B1)(C1,B) \tkzGetPoint{F}
            \tkzInterLL(A,C1)(A1,C) \tkzGetPoint{E}
            \tkzInterLL(A1,B)(C,B1) \tkzGetPoint{Y}
            \tkzInterLL(A1,C)(A,B1) \tkzGetPoint{Z}
            \draw[dotted, line width=.2mm] (A1) -- (Y) -- (B1) -- (A) -- (C1) -- (B);
            \draw[dotted, line width=.2mm] (A1) -- (C);
            \tkzDrawSegment[add=0 and .15](X,C)
            \tkzDrawSegment[add=0 and .1](X,C1)
            \tkzDrawSegment[add=.5 and .5](D,F)
            \tkzLabelPoints[above left](A,B)
            \tkzLabelPoint[left](X){$X$}
            \tkzLabelPoint[above](Y){$Y$}
            \tkzLabelPoint[below](Z){$Z$}
            \tkzLabelPoint[above](E){$E$}
            \tkzLabelPoint[above left=-.1](D){$D$}
            \tkzLabelPoint[above left=-.1](C){$C$}
            \tkzLabelPoint[above right](F){$F$}
            \tkzLabelPoint[below](A1){$A_1$}
            \tkzLabelPoint[below](B1){$B_1$}
            \tkzLabelPoint[below](C1){$C_1$}
            \tkzDrawPoints(A,B,C,D,E,F,A1,B1,C1,X,Y,Z)
        \end{tikzpicture}
    \end{center}
    Пусть $\hat{E}=DF\cap AC_1$, $X=AB\cap A_1B_1$, $Y=A_1B\cap B_1C$, $Z=A_1C\cap AB_1$. Рассотрим центральные проекции: из $A$ прямой $A_1C$ на $A_1C_1$: $(A_1,Z;E,C)=(A_1,B_1;C_1,X)$; из $B$ прямой $A_1B$ на $B_1C$: $(A_1,B_1;C_1,X)=(Y,B_1;F,C)$; из $D$ прямой $B_1Y$ на $A_1C$: $(Y,B_1;F,C)=(A_1,Z;\hat{E},C)$, то есть $(A_1,Z;E,C)=(A_1,Z;\hat{E},C)\Longrightarrow\hat{E}\equiv E$, а значит $D$, $E$ и $F$ коллинеарны.
\end{proof}
...
\begin{axiom}[Аксиома расстояния]
    ...
    $$A \neq B;\,\forall\,\alpha:A,\,B\in\alpha\enspace\rho_\alpha(A;B)=\text{const}$$
\end{axiom}
\begin{definition}
...
\begin{lemma}
    ...
    $$\forall\,A;\,\forall\,l:\,A\notin l\enspace\exists!\,\alpha:\,l\subset\alpha;\,A\in\alpha$$
\end{lemma}
...
\begin{proof}
    ...
    \begin{center}
        \begin{tikzpicture}[z={(0:10mm)},x={(25:5mm)},y={(110:7mm)}]
            ...
            \tkzDrawPoints(A, B, C, A1, B1, C1, H, H1, K, N, M)
          \end{tikzpicture}
    \end{center}
...
\end{statement}
\begin{proof}
  $$V=2\int_{0}^{R}\pi\left(R^2-(R-r)^2\right)\,d r=4\pi r\int_{0}^{R}\,d r-2\pi\int_{0}^{R}r^2\,d r=4\pi R\cdot \frac{R^2}{2} - 2\pi\cdot \frac{R^3}{3}=\frac{4}{3}\pi R^3$$
\end{proof}
\begin{definition}
...
\begin{proof}
  ...
  $$V=\int_{-R}^{R}(\pi x_2^2-\pi x_1^2)\,d y=\pi\int_{-R}^{R}2d\cdot2\sqrt{R^2-y^2}\cdot d y=4\pi d\int_{-R}^{R}\sqrt{R^2-y^2}\cdot d y=2\pi^2 d R^2$$
\end{proof}
\begin{definition}
...
\end{statement}
\begin{definition}
	Смешанным произведением векторов $\vv{a}$, $\vv{b}$ и $\vv{c}$ называется $\vv{a}\cdot\left(\vv{b}\times\vv{c}\right)$.
\end{definition}
\begin{statement}[Свойства векторного произведения]
	\hfill
	\begin{enumerate}
...
\begin{definition}[Предел функции]
	\hfill
	$$A=\lim_{x\to x_0}f(x)\Longleftrightarrow \forall \varepsilon >0\enspace \exists\, \delta >0:\,|x-x_0|<\delta \Longrightarrow |f(x)-A|<\varepsilon$$
\end{definition}
\begin{definition}[Правосторонний предел функции]
	\hfill
	$$A=\lim_{x\to x_0+0}f(x)\Longleftrightarrow \forall \varepsilon >0\enspace \exists\, \delta >0:\,\forall x \in (x_0; x_0 + \delta)\Longrightarrow |f(x)-A|<\varepsilon$$
\end{definition}
\begin{definition}[Левосторонний предел функции]
	\hfill
	$$A=\lim_{x\to x_0-0}f(x)\Longleftrightarrow \forall \varepsilon >0\enspace \exists\, \delta >0:\,\forall x \in (x_0-\delta; x_0)\Longrightarrow |f(x)-A|<\varepsilon$$
\end{definition}
\begin{definition}
...
\begin{proof}
	...
	$$V_{PA_1A_2\ldots A_n}=\frac{1}{3}S_{A_1A_2\ldots A_n}\frac{H}{2};\quad V_{PB_1B_2\ldots B_m}=\frac{1}{3}S_{B_1B_2\ldots B_m}\frac{H}{2}$$
    ...
    \begin{center}
        \begin{tikzpicture}[3d view={-160}{-7},scale=0.8]
            ...
            \tkzDrawPoints(A1, B1, B2, C1, C2, P, H)
		\end{tikzpicture}
    \end{center}
