...
\begin{definition}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw (-2,0) node[below left] {$-1$};
            \draw (2,0) node[below right] {$1$};
            \tkzLabelAngle(B,O,A){$\alpha$};
            \tkzMarkAngle[size=.55cm](B,O,A);
            \draw (O) node[below left] {$O$};
            \draw[-{Stealth[scale = 1.5]}] (-3,0) -- (5, 0) node[below] {$\cos \alpha$};
            \draw[-{Stealth[scale = 1.5]}] (0,-3) -- (0,5) node[left] {$\sin \alpha$};
            \draw[-{Stealth[scale = 1.5]}] (-3,2) -- (5, 2) node[below] {$\ctg \alpha$};
            \draw[-{Stealth[scale = 1.5]}] (2,-3) -- (2,5) node[left] {$\tg \alpha$};
            \foreach \point in {(O), (A), (0,2), (0,-2), (-2,0), (2,0)}{
    \fill \point circle (1.8pt);
}
        \end{tikzpicture}
    \end{center}
...
\begin{proof}
    ...
    \end{statement}
    \begin{proof}
        $$\tg(\alpha+\beta)=\dfrac{\cos(\alpha+\beta)}{\sin(\alpha+\beta)}=\dfrac{\cos\alpha\cdot\cos\beta-\sin\alpha\cdot\sin\beta}{\sin\alpha\cdot\cos\beta+\cos\alpha\cdot\sin\beta}$$
