...
\begin{definition}
    ...
    Где $\alpha_{x_0}(x)$ -- бесконечно малая в $x_0$ функция, число $c$ -- производная $f(x)$ в точке $x_0$.
\end{definition}
\begin{statement}
...
\end{proof}
\begin{definition}
    Пусть $f(x)$ дифференцируема во всех точках множества $M$. Прозводной функцией $f(x)$ называется функция $f'(x)$, такая что её значение в каждой точке $M$ является производной $f(x)$ в данной точке.
\end{definition}
\begin{statement}
...
\end{statement}
\begin{proof}
    $$\lim_{x\to x_0}\frac{\tg x-\tg x_0}{x-x_0}=\lim_{x\to x_0}\frac{\sin(x-x_0)}{\cos x\cos x_0 (x-x_0)}=\frac{1}{\cos^2x}$$
\end{proof}
\begin{statement}
    Пусть $f(x)=\ctg x$, тогда $f'(x)=-\dfrac{1}{\sin^2x}$.
\end{statement}
\begin{proof}
    $$\lim_{x\to x_0}\frac{\ctg x-\ctg x_0}{x-x_0}=\lim_{x\to x_0}\frac{-\sin(x-x_0)}{\sin x\sin x_0 (x-x_0)}=-\frac{1}{\sin^2x}$$
\end{proof}
\begin{statement}
    Пусть $f(x)=\sqrt{x}$, тогда $f'(x)=\dfrac{1}{2\sqrt{x}}$.
\end{statement}
\begin{proof}
    $$\lim_{x\to x_0}\frac{\sqrt{x}-\sqrt{x_0}}{x-x_0}=\lim_{x\to x_0}\frac{1}{\sqrt{x}+\sqrt{x_0}}=\frac{1}{2\sqrt{x}}$$
\end{proof}
